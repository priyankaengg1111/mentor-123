...
module.exports = {
    ...
    remove: function (req, res) {
        ...
        agentModel.findByIdAndRemove(id, function(err) {
            if (err) {
                return res.status(500).json({
                    message: 'Error when deleting the agent.',
                    error: err
                });
            }
    ...
    register: async function(req,res){
        ...
        var agent = await agentModel.findOne({ email: body.email });
        
        // if there is any agent or not
        if(agent){
        ...
        // check if it is able to save agent
        try{
            const message = {
            ...
            console.log(err);  
                if (err) {
                    ...
                    try{
                        ...
                        console.log(e)
                        return res.status(400).send({success : false, message: "Unable to save agent" });
                    }
                }
            });
        } 
    ...
    forgotPassword :async function(req,res){
        ...
        agentModel.findOne({email:req.body.email},async (err,agent)=>{
            if(err){
                return res.status(401).json({
                    success:false,
                    messages:"DB Error"
            ...
            try{
                const message = {
                    ...
                    text: `Hello ${agent.name}, Your New password is ${password}` // Plain text body
                };
                transport.sendMail(message, async function(err, info) {
                    ...
                    if (err) {
                        ...
                        try{
                            await agent.save();                
                            return res.status(200).send({ 
                                success : true,message: "agent Password Changed"
                            });
                        }            
                        catch(e){
                            console.log(e)
    ...
    login : async function(req,res){
        ...
        try{
            const agent = await agentModel.findOne({ email: body.email});
            ...
            if(agent.password != body.password){
                return res.status(201).json({
                    success : false,
    ...
    changePassword:function(req,res){
        ...
        agentModel.findOne({_id:agentData._id,password:agentData.password},async (err,agent)=>{
            if(err){
                res.status(201).json({
                    error:err,
            ...
            if(!agent){
                res.status(401).json({
                    success : false, message: "No agent" });
            }
